...
> If accessing from another PC/Phone via HTTP (`http://192.168.x.x`), the camera will be blocked by the browser.
> You must enable `chrome://flags/#unsafely-treat-insecure-origin-as-secure` on the client device and add the server URL.
